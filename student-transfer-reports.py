...
def main(filename):
    ...
    students = [student for student in students \
        if str(student['STUDENT_CURRENT_SCHOOL_CODE']).isdigit() \
        and str(student['STUDENT_NEXT_OR_PREV_YEAR_SCHOOL_CODE']).isdigit()]
...
def make_dicts(students):
    ...
    for school in schools:
        directory[school['school_id']] = school['short_name']
    ...
    for student in students:
        if student:
            ...
            if middle_school_id not in directory:
    ...
    for index in schools:
        ...
        with open('reports/' + report_type + '/csv_reports/' + name + '.csv', 'w') as f:
            ...
            for key in sorted(school, key=school.__getitem__):
                if key[0].isdigit() and names[key] != '---':
